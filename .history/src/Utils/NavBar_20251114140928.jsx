...
function NavBar() {
  ...
  if (
    window.localStorage.getItem("moneySystem") &&
    window.localStorage.moneySystem !== null
  ) {
  ...
  function AllActivity() {
    ...
    if (
      security.change === false &&
      security.add === false &&
      security.translate === false
    ) {
      setError({
        state: true,
        massage: [{ ms: "Please Active One Switch To Work", type: "err" }],
      });
    }
    ...
    if (security.change) {
      ChangeMoney();
      setError({
        state: error.state ? true : false,
        massage: [...error.massage, { ms: "Success Change Work", type: "suc" }],
      });
    }
    ...
    if (security.add) {
      if (addMoney <= 0) {
        setError({
          ...
          massage: [
            ...error.massage,
            { ms: "Please we not allow 0 or litter than!", type: "err" },
          ],
        });
      } else {
        setError({
          state: error.state ? true : false,
          massage: [...error.massage, { ms: "Success Add Work", type: "suc" }],
        });
        PlusMoney();
      }
    }
    ...
    if (security.translate) {
      if (+translate > +money.sells || +translate <= 0) {
        setError({
          ...
          massage: [
            ...error.massage,
            { ms: "The Translate has an error", type: "err" },
          ],
        });
      } else {
        setError({
          ...
          massage: [
            ...error.massage,
            { ms: "Success Translate Work", type: "suc" },
          ],
        });
        TransSToMoney();
      }
    ...
    if (
      (error.state === false && security.add === true) ||
      security.change === true ||
      security.translate === true
    ) {
      handleClose();
    }
  }
  ...
  return (
    <div className="NavBar">
      ...
      <div className="Additional">
        <Nav variant="dark" bg="dark" className="language-step">
          <Dropdown data-bs-theme="dark">
            ...
            <Dropdown.Menu>
              ...
              >
                {lg === "en" && <MdOutlineDone className="text-success" />}{" "}
                English
              </Dropdown.Item>
              <Dropdown.Item
                onClick={() => {
              ...
              >
                {lg === "ar" && <MdOutlineDone className="text-success" />}{" "}
                Arabic
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
        ...
        <span
          className="Link-Additional btn btn-dark money-system-step"
          onClick={ShowModF}
        >
          <FaMoneyCheckDollar />
        </span>

        <Link to="/" className="Link-Additional btn btn-dark">
          <IoHome />
        </Link>
        <a
          href="https://portfolio-abdelghafar.vercel.app/"
          className="Link-Additional btn btn-dark"
          title="my portfolio"
        >
          <FaInfoCircle />
        </a>
      ...
      >
        ...
        <Modal.Body>
          <Form>
            <div className="d-flex justify-content-end align-items-center">
              ...
              <Form.Check
                ...
                onClick={() =>
                  security.change
                    ? setSecurity({ ...security, change: false })
                    : setSecurity({ ...security, change: true })
                }
            ...
            <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
              ...
              <Form.Control
                ...
                className={
                  !security.change
                    ? "bg-light text-success text-center fw-bolder"
                    : ""
                }
            ...
            >
              <div className="d-flex justify-content-end align-items-center">
                <Form.Check
                  ...
                  onClick={() =>
                    security.add
                      ? setSecurity({ ...security, add: false })
                      : setSecurity({ ...security, add: true })
                  }
                />
              </div>
            ...
            <div className="d-flex justify-content-end align-items-center">
              <Form.Check
                ...
                onClick={() =>
                  security.translate
                    ? setSecurity({ ...security, translate: false })
                    : setSecurity({ ...security, translate: true })
                }
              />
            </div>

            <h3 className="text-center p-2 bg-light fw-lighter">
              {lang.MoneySys.titles.transTitle}
            </h3>
            ...
            <Row className="g-2">
              <Col md>
                <FloatingLabel
                  ...
                  label={lang.MoneySys.inputs.sells}
                >
                  <Form.Control
                    type="number"
                    className="bg-light"
                    placeholder=""
                    value={money.sells - translate}
                    readOnly
                  />
                </FloatingLabel>
              </Col>
              ...
              <Col md>
                <FloatingLabel label={lang.MoneySys.inputs.transMoney}>
                  <Form.Control
                    className={`${
                      security.translate
                        ? +money.sells < +translate
                          ? "border-danger text-danger"
                          : ""
                        : "bg-light"
                    }`}
                    readOnly={security.translate ? false : true}
                    type="number"
                    placeholder="Money Translate"
                    value={translate}
          ...
          <div className="">
            {error.state
              ? error.massage.map((err) => {
                  return (
                    <p
                      className={`mt-3 alert ${
                        err.type === "err" ? "alert-danger" : "alert-success"
                      }`}
                    >
                      {err.type === "err" ? (
                        <IoIosCloseCircle />
